...
// Assign seats for Steffen Modified

function assignSeats() {
    var numSeatsInRowPerSide = numSeatsInRow/2
    ...
    while (unassignedPassengers.length != 0) {
        ...
        for (r=0;r<numSeatsInRow-3;r++) { // for rows 1 to 3 
            for (count=(r+1)*numRowSeats-1;count>=r*numRowSeats;count=count-2) { // for every odd column from 1 to 35
                var idx = Math.floor(Math.random()*(numSeatsInGroups1_2*numSeatsInRowPerSide-delcount));
        ...
        for (r=3;r<numSeatsInRow;r++) { // for rows 4 to 6
            for (count=(r+1)*numRowSeats-1;count>=r*numRowSeats;count=count-2) { // for every odd column from 1 to 35
                var idx = Math.floor(Math.random()*(numSeatsInGroups1_2*numSeatsInRowPerSide-delcount2));
                seatMatrix[count].passengerID = unassignedPassengers[idx];
                unassignedPassengers.splice(idx, 1);
                delcount2++;
            }
        ...
        for (r=0;r<numSeatsInRow-3;r++) { // for rows 1 to 3 
            for (count=(r+1)*numRowSeats-2;count>=r*numRowSeats+1;count=count-2) { // for every even column from 1 to 35
                var idx = Math.floor(Math.random()*(numSeatsInGroups3_4*numSeatsInRowPerSide-delcount3));
                seatMatrix[count].passengerID = unassignedPassengers[idx];
                unassignedPassengers.splice(idx, 1);
                delcount3++;
            }
        ...
        for (r=3;r<numSeatsInRow;r++) { // for rows 4 to 6
            for (count=(r+1)*numRowSeats-2;count>=r*numRowSeats+1;count=count-2) { // for every even column from 1 to 35
                var idx = Math.floor(Math.random()*(numSeatsInGroups3_4*numSeatsInRowPerSide-delcount4));
                seatMatrix[count].passengerID = unassignedPassengers[idx];
                unassignedPassengers.splice(idx, 1);
                delcount4++;
            }
