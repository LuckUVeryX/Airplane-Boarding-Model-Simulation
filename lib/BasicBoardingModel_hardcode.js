...
function assignSeats() {
    ...
    // #####################################################################################
    // Jordan
    // 2 varable to keep in mind: numRowSeats and numSeatsInRow and numPassengers
    unassignedPassengers = [];
    passengner_count = 0;
    for (i = 1; i < numPassengers; i++) {
        unassignedPassengers.push(i);
    ...
    // variable for 6th odd row
    var row6_oddindedxseat = numPassengers -1 
    //need to -1 as index start at 0 and ends at 209
    // variable for 6th even row
    var row6_evenindexseat = numPassengers -2

    //variable for 5th odd row
    var row5_oddindedxseat = numPassengers -1 - numRowSeats
    //variable for 5th even row
    var row5_evenindedxseat = numPassengers -2 - numRowSeats

    
    while (unassignedPassengers.length != 0) {
        
        // 1st grp  Seat6 ODD Index seats 
        if (row6_oddindedxseat >= numPassengers-numRowSeats) {
            seatMatrix[row6_oddindedxseat].passengerID = unassignedPassengers[0];
            unassignedPassengers.splice(0, 1);
            row6_oddindedxseat -=2;
            passengner_count++;
        }
        
        //3rd grp Seat6 EVEN Index seats
        else if(row6_evenindexseat > numPassengers-1-numRowSeats)
        // As would include one more passenger to be added to next row
        {
            seatMatrix[row6_evenindexseat].passengerID = unassignedPassengers[0];
            unassignedPassengers.splice(0, 1);
            row6_evenindexseat -=2;
            passengner_count++;
            // console.log(numPassengers-1-numRowSeats)
        }

        // 5th  grp Seat5 ODD Index seats 
        else if(row5_oddindedxseat >= numPassengers-2*numRowSeats)
        // As would include one more passenger to be added to next row
        {
            seatMatrix[row5_oddindedxseat].passengerID = unassignedPassengers[0];
            unassignedPassengers.splice(0, 1);
            row5_oddindedxseat -=2;
            passengner_count++;
        }

        // 7th  grp Seat5 EVEN Index seats 
        else if(row5_evenindedxseat > numPassengers-1-2*numRowSeats)
        // As would include one more passenger to be added to next row
        {
            seatMatrix[row5_evenindedxseat].passengerID = unassignedPassengers[0];
            unassignedPassengers.splice(0, 1);
            row5_evenindedxseat -=2;
            passengner_count++;
        }        


        else{
            //this is to help check the logic, can remove later
            var idx = Math.floor(Math.random() * unassignedPassengers.length);
            if (seatMatrix[idx].passengerID != -1 )
                seatMatrix[idx].passengerID = unassignedPassengers[0];
                unassignedPassengers.splice(idx, 1);
                passengner_count++; 
        }
