...
function addDynamicAgents(counter) {
    ...
    if (idCount <= numPassengers && aisleList[queueStartCol] == EMPTY && counter % INTERVAL == 0) {
        ...
        var newPassenger = {
            ...
            "baggageStoringTime": BAGGAGESTORELOWERLIMIT + Math.floor(Math.random() * Math.floor(BAGGAGESTOREUPPERLIMIT - BAGGAGESTORELOWERLIMIT)), // Uniform Distribution [BAGGAGESTORELOWERLIMIT, BAGGESTOREUPPERLIMIT]
...
function getStatistics() {
    ...
    if (seatedPassengers.length < numPassengers) {
        ...
        statistics[7].cumulativeValue = simulationNum + 1;
