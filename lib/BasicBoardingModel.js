...
const urlPassengerSeated = "images/Superintendant_seated.png";
...
var areas = [
    ...
    { "label": "Aisle", "startRow": aisleStartRow, "numRows": 1, "startCol": aisleStartCol, "numCols": numRowSeats, "color": "#9FA9FF" }
]
...
var statistics = [
    { "name": "Total time taken: ", "location": { "row": 12.5, "col": 3 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Average boarding time: ", "location": { "row": 13.5, "col": 3 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Seat shuffles: ", "location": { "row": 14.5, "col": 3 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Stops for bag stowing: ", "location": { "row": 15.5, "col": 3 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Cumulative time spent waiting: ", "location": { "row": 16.5, "col": 3 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Mean bag store time: ", "location": { "row": 12.5, "col": 19 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Mean seat shuffle time: ", "location": { "row": 13.5, "col": 19 }, "cumulativeValue": 0, "count": 0 },
    { "name": "Simulation Run: ", "location": { "row": 15.5, "col": 19 }, "cumulativeValue": 0, "count": 0 }
];

// Arrays to hold statistics for multiple simulation runs
var multiTotalBoardingTimeStatistic = ["totalBoardingTime"];
var multiAvgPassengerBoardingTimeStatistic1 = ["avgBoardingTime"];
var seatShuffleNumberStatistic = ["seatShuffle"];
var waitNumberStatistic = ["numStops"];
var cumWaitStatistic = ["cumWaitTime"];

var simulationNum = 0;

const MODELS = ["random", "ftb", "btf", "wta_random", "wta_ftb", "wta_btf", "sm", "sp"];
var currentModel = 0;
var statsTable = [];
...
const INTERVAL = 3; // Number of sim steps between passenger spawns
...
function initialiseVariables() {
    ...
    statistics[0].cumulativeValue = 0; // Total time taken
    statistics[1].cumulativeValue = 0; // Avg boarding time
    statistics[2].cumulativeValue = 0; // Seat Shuffles
    statistics[3].cumulativeValue = 0; // Stops for bag stowing
    statistics[4].cumulativeValue = 0; // Cumulative waiting time

    statistics[0].count = 0; // Total time taken
    statistics[1].count = 0; // Avg boarding time
    statistics[2].count = 0; // Seat Shuffles
    statistics[3].count = 0; // Stops for bag stowing
    statistics[4].count = 0; // Cumulative waiting time

    statistics[5].cumulativeValue = (BAGGAGESTORELOWERLIMIT + BAGGAGESTOREUPPERLIMIT) / 2; // Mean baggage store time
    statistics[6].cumulativeValue = SHUFFLETIME; // Mean seat shuffle time
    statistics[7].cumulativeValue = 0; // Simulation Run
    statistics[5].count = 0; // Mean baggage store time
    statistics[6].count = 0; // Mean seat shuffle time
    statistics[7].count = 0; // Simulation Run
...
function redrawWindow() {
    ...
    animationDelay = 700 - document.getElementById("slider1").value;
    simTimer = window.setInterval(simStep, animationDelay); // call the function simStep every animationDelay milliseconds
    ...
    window.BAGGAGESTORELOWERLIMIT = Math.max(parseInt(document.getElementById("bagstoringtime").value) - 3, 0);
    window.BAGGAGESTOREUPPERLIMIT = Math.min(parseInt(document.getElementById("bagstoringtime").value) + 3, document.getElementById("bagstoringtime").value * 2 - BAGGAGESTORELOWERLIMIT);
...
function updateSurface() {
    ...
    images.transition()
        ...
        .attr("xlink:href", function (d) {
            ...
            } else if (d.state == SHUFFLING & d.seatShuffleTime != 0) {
                return urlPassengerShuffling;
            } else if (d.state == SEATED) {
                return urlPassengerSeated;
            } else {
                return urlPassenger;
            }
        })
...
function addDynamicAgents(counter) {
    // counter % INTERVAL controls the intervals when we add new passengers
    if (idCount <= numPassengers && aisleList[queueStartCol] == EMPTY && counter % INTERVAL == 0) {
        ...
        var newPassenger = {
            ...
            // Normal Distribution [BAGGAGESTORELOWERLIMIT, BAGGESTOREUPPERLIMIT]
            "baggageStoringTime": Math.round(randn_bm(BAGGAGESTORELOWERLIMIT, BAGGAGESTOREUPPERLIMIT, 1)),
...
function getStatistics() {
    ...
    if (seatedPassengers.length < numPassengers) {
        statistics[0].cumulativeValue = currentTime; // Total time taken stat
        statistics[7].cumulativeValue = simulationNum + 1; // Simulation number stat
        ...
        cumWaitStatistic.push(statistics[4].cumulativeValue / Math.max(1, statistics[4].count));



        if (simulationNum != TOTALNUMSIMS) { // We need to simulate more runs
            redrawWindow();
        }
    ...
    } if (simulationNum >= TOTALNUMSIMS) { // Once all simulation runs are completed
        isRunning = false;

        // Print statistics
        console.log("Boarding Model: " + MODELS[currentModel]);
        ...
        console.log("Cumulative waits: ");
        console.log(cumWaitStatistic);

        for (i = 0; i <= TOTALNUMSIMS; i++) {
            statsTable.push([
                multiTotalBoardingTimeStatistic[i],
                multiAvgPassengerBoardingTimeStatistic1[i],
                seatShuffleNumberStatistic[i],
                waitNumberStatistic[i],
                cumWaitStatistic[i],
            ])
        }
        ...
        let csvContent = "data:text/csv;charset=utf-8,"
            + statsTable.map(e => e.join(",")).join("\n");

        var encodedUri = encodeURI(csvContent);
        var link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", `bag_${(BAGGAGESTORELOWERLIMIT + BAGGAGESTOREUPPERLIMIT) / 2}_seat_${SHUFFLETIME}_${MODELS[currentModel]}.csv`);
        document.body.appendChild(link); // Required for FF

        link.click(); // This will download the data file named "my_data.csv".

        // Reset Multiple Simulation Statistics
        multiTotalBoardingTimeStatistic = ["totalBoardingTime"];
        multiAvgPassengerBoardingTimeStatistic1 = ["avgBoardingTime"];
        seatShuffleNumberStatistic = ["seatShuffle"];
        waitNumberStatistic = ["numStops"];
        cumWaitStatistic = ["cumWaitTime"];
        ...
        if (currentModel < MODELS.length - 1) {
            ...
            resetSimulation();
            isRunning = true;
        } else { // stop when all models are completed
            isRunning = false;
            currentModel = 0;
        }
    }
...
function updatePassenger(passengerIndex) {
    ...
    switch (state) {
        case WAITING:
            ...
            // count the number of simsteps where passenger is in waiting state
            statistics[4].cumulativeValue++;
            // update passenger's current location to OCCUPIED
        ...
        case QUEUEING:
            ...
            if (aisleList[col + 1] == EMPTY) {
                ...
                aisleList[col] = EMPTY; // Update aisleList as passenger has left previous location in aisle
            } else {
                // wait for next place in aisleList to be empty.
        ...
        case STORING:
            // Update aisleList that there is a passenger at current location
            aisleList[col] = OCCUPIED;
            // wait baggageStoringTime for baggage storing to be complete
            // if baggageStoringTime amount of time has passed, change state to SEATING
        ...
        case SHUFFLING:
            ...
            if (passenger.isSeatShuffleCalculated == NOTCALCULATED) {
                ...
                // if passenger is seated top side
                if (passenger.seatArea == TOPAREA) {
                    // we need to check if there are passengers already seated below his assigned seat
                    ...
                    // if passenger is seated bottom side
                } else if (passenger.seatArea == BTMAREA) {
                    // we need to check if there are passengers already seated above his assigned seat
                    while (passenger.seatChecker - numRowSeats > numPassengers / 2) {
                        passenger.seatsToCheck.push(passenger.seatChecker - numRowSeats);
                        passenger.seatChecker -= numRowSeats;
                    }
                ...
                // to ensure that we only calculate seat shuffle once
